...
type EventQueue interface {
	...
	Connect(ctx context.Context, queueId string, lastEventId int64) (<-chan Event, error)
...
type eventQueue struct {
	client       real_time_events.APIRealTimeEvents
...
func NewEventQueue(client real_time_events.APIRealTimeEvents, handler EventQueueErrorHandler) *eventQueue {
	return &eventQueue{
		client:       client,
		lastEventId:  atomic.Int64{},
...
func (q *eventQueue) Connect(ctx context.Context, queueId string, lastEventId int64) (<-chan Event, error) {
	...
	events := make(chan Event, 32) // default buffer size
...
func (q *eventQueue) pollEvents(ctx context.Context, events chan<- Event) {
	defer close(events)
...
func (q *eventQueue) processEvents(ctx context.Context, resp *real_time_events.GetEventsResponse, events chan<- Event) bool {
