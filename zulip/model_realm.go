...
type Realm struct {
	...
	// Dictionary where each entry describes a supported [video call provider](https://zulip.com/help/configure-call-provider) that is configured on this server and could be selected by an organization administrator.  Useful for administrative settings UI that allows changing the realm setting `video_chat_provider`.
	AvailableVideoChatProviders map[string]VideoChatProviderInfo `json:"realm_available_video_chat_providers,omitempty"`
	// Whether this Zulip server is configured to allow organizations to enable [digest emails](https://zulip.com/help/digest-emails).  Relevant for administrative settings UI that can change the digest email settings.
	...
	// Whether the organization allows any sort of password-based authentication (whether via EmailAuthBackend or LDAP passwords).  Determines whether a client might ever need to display a password prompt (clients will primarily look at this attribute in [server_settings](https://zulip.com/api/get-server-settings) before presenting a login page).
	...
	// Dictionary where each entry describes a default external account type that can be configured with Zulip's [custom profile fields feature](https://zulip.com/help/custom-profile-fields).  **Changes**: New in Zulip 2.1.0.
	...
	// Recommended client-side HTTP request timeout for [`GET /events`](https://zulip.com/api/get-events) calls. This is guaranteed to be somewhat greater than the heartbeat frequency. It is important that clients respect this parameter, so that increases in the heartbeat frequency do not break clients.  **Changes**: New in Zulip 5.0 (feature level 74). Previously, this was hardcoded to 90 seconds, and clients should use that as a fallback value when interacting with servers where this field is not present.
...
type RealmConfiguration struct {
	...
	// Whether [new users joining](https://zulip.com/help/restrict-account-creation#configuring-email-domain-restrictions) this organization are required to have an email address in one of the `realm_domains` configured for the organization.
	...
	// The organization's default policy for sending channel messages to the [empty \"general chat\" topic](https://zulip.com/help/general-chat-topic).  - `\"allow_empty_topic\"`: Channel messages can be sent to the empty topic. - `\"disable_empty_topic\"`: Channel messages cannot be sent to the empty topic.  **Changes**: New in Zulip 11.0 (feature level 392). Previously, this was controlled by the boolean realm `mandatory_topics` setting, which is now deprecated.
	TopicsPolicy TopicsPolicy `json:"topics_policy,omitempty"`
	// Whether [topics are required](https://zulip.com/help/require-topics) for messages in this organization.  **Changes**: Deprecated in Zulip 11.0 (feature level 392). This is now controlled by the realm `topics_policy` setting.
	// Deprecated
	...
	// Messages sent more than this many seconds ago cannot be deleted with this organization's [message deletion policy](https://zulip.com/help/restrict-message-editing-and-deletion).  Will not be 0. A `null` value means no limit: messages can be deleted regardless of how long ago they were sent.  **Changes**: No limit was represented using the special value `0` before Zulip 5.0 (feature level 100).
	MessageContentDeleteLimitSeconds *int64 `json:"message_content_delete_limit_seconds,omitempty"`
	// Messages sent more than this many seconds ago cannot be edited with this organization's [message edit policy](https://zulip.com/help/restrict-message-editing-and-deletion).  Will not be `0`. A `null` value means no limit, so messages can be edited regardless of how long ago they were sent.  See [`PATCH /messages/{message_id}`](https://zulip.com/api/update-message) for details and history of how message editing permissions work.  **Changes**: Before Zulip 6.0 (feature level 138), no limit was represented using the special value `0`.
	MessageContentEditLimitSeconds *int64 `json:"message_content_edit_limit_seconds,omitempty"`
	// Which type of message edit history is configured to allow users to access [message edit history](https://zulip.com/help/view-a-messages-edit-history).  - \"all\" = All edit history is visible. - \"moves\" = Only moves are visible. - \"none\" = No edit history is visible.  **Changes**: New in Zulip 10.0 (feature level 358), replacing the previous `allow_edit_history` boolean setting; `true` corresponds to `all`, and `false` to `none`.
	MessageEditHistoryVisibilityPolicy string `json:"message_edit_history_visibility_policy,omitempty"`
	// Messages sent more than this many seconds ago cannot be moved within a channel to another topic by users who have permission to do so based on this organization's [topic edit policy](https://zulip.com/help/restrict-moving-messages). This setting does not affect moderators and administrators.  Will not be `0`. A `null` value means no limit, so message topics can be edited regardless of how long ago they were sent.  See [`PATCH /messages/{message_id}`](https://zulip.com/api/update-message) for details and history of how message editing permissions work.  **Changes**: New in Zulip 7.0 (feature level 162). Previously, this time limit was always 72 hours for users who were not administrators or moderators.
	MoveMessagesWithinStreamLimitSeconds *int64 `json:"move_messages_within_stream_limit_seconds,omitempty"`
	// Messages sent more than this many seconds ago cannot be moved between channels by users who have permission to do so based on this organization's [message move policy](https://zulip.com/help/restrict-moving-messages). This setting does not affect moderators and administrators.  Will not be `0`. A `null` value means no limit, so messages can be moved regardless of how long ago they were sent.  See [`PATCH /messages/{message_id}`](https://zulip.com/api/update-message) for details and history of how message editing permissions work.  **Changes**: New in Zulip 7.0 (feature level 162). Previously, there was no time limit for moving messages between channels for users with permission to do so.
	...
	// Whether or not this organization is configured to send the standard Zulip [welcome emails](https://zulip.com/help/disable-welcome-emails) to new users joining the organization.
	...
	// Members whose accounts have been created at least this many days ago will be treated as [full members][calc-full-member] for the purpose of settings that restrict access to new members.  [calc-full-member]: https://zulip.com/api/roles-and-permissions#determining-if-a-user-is-a-full-member
	WaitingPeriodThreshold int64 `json:"waiting_period_threshold,omitempty"`
	// Whether the organization has given permission to be advertised in the Zulip [communities directory](https://zulip.com/help/communities-directory).  **Changes**: New in Zulip 6.0 (feature level 129).
...
type RealmPresentation struct {
	...
	// String indicating whether the organization's [profile wide logo](https://zulip.com/help/create-your-organization-profile) was uploaded by a user or is the default. Useful for UI allowing editing the organization's wide logo.  - \"D\" means the logo is the default Zulip logo. - \"U\" means uploaded by an organization administrator.
	LogoSource string `json:"logo_source,omitempty"`
	// The URL of the organization's wide logo configured in the [organization profile](https://zulip.com/help/create-your-organization-profile).
	LogoUrl string `json:"logo_url,omitempty"`
	// String indicating whether the organization's dark theme [profile wide logo](https://zulip.com/help/create-your-organization-profile) was uploaded by a user or is the default. Useful for UI allowing editing the organization's wide logo.  - \"D\" means the logo is the default Zulip logo. - \"U\" means uploaded by an organization administrator.
	NightLogoSource string `json:"night_logo_source,omitempty"`
	// The URL of the organization's dark theme wide-format logo configured in the [organization profile](https://zulip.com/help/create-your-organization-profile).
	...
	// String indicating whether the organization's [profile icon](https://zulip.com/help/create-your-organization-profile) was uploaded by a user or is the default. Useful for UI allowing editing the organization's icon.  - \"G\" means generated by Gravatar (the default). - \"U\" means uploaded by an organization administrator.
	IconSource string `json:"icon_source,omitempty"`
	// The URL of the organization's [profile icon](https://zulip.com/help/create-your-organization-profile).
	IconUrl string `json:"icon_url,omitempty"`
	// Whether this organization has been configured to enable [previews of linked images](https://zulip.com/help/image-video-and-website-previews).
	InlineImagePreview bool `json:"inline_image_preview,omitempty"`
	// Whether this organization has been configured to enable [previews of linked websites](https://zulip.com/help/image-video-and-website-previews).
	InlineUrlEmbedPreview bool `json:"inline_url_embed_preview,omitempty"`
}

type RealmLocalization struct {
	// The default pygments language code to be used for code blocks in this organization. If an empty string, no default has been set.  **Changes**: Prior to Zulip 8.0 (feature level 195), a server bug meant that both `null` and an empty string could represent that no default was set for this realm setting in the [`POST /register`](https://zulip.com/api/register-queue) response. The documentation for both that endpoint and this event incorrectly stated that the only representation for no default language was `null`. This event in fact uses the empty string to indicate that no default has been set in all server versions.
...
type RealmPermissions struct {
	// Indicates whether users are [allowed to change](https://zulip.com/help/restrict-name-and-email-changes) their name via the Zulip UI in this organization. Typically disabled in organizations syncing this type of account information from an external user database like LDAP.
	...
	// Whether this organization's [message edit policy][config-message-editing] allows editing the content of messages.  See [`PATCH /messages/{message_id}`](https://zulip.com/api/update-message) for details and history of how message editing permissions work.  [config-message-editing]: https://zulip.com/help/restrict-message-editing-and-deletion
	...
	// A [group-setting value](https://zulip.com/api/group-setting-values) defining the set of users who have permission to delete any message in the organization.  **Changes**: New in Zulip 10.0 (feature level 281). Previously, this permission was limited to administrators only and was uneditable.
	...
	// A [group-setting value](https://zulip.com/api/group-setting-values) defining the set of users who have permission to create user groups in this organization.  **Changes**: New in Zulip 10.0 (feature level 299). Previously `realm_user_group_edit_policy` field used to control the permission to create user groups.
	...
	// A [group-setting value](https://zulip.com/api/group-setting-values) defining the set of users who are allowed to use AI summarization.  **Changes**: New in Zulip 10.0 (feature level 350).
