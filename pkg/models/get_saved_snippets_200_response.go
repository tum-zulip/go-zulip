/*
Zulip REST API

Powerful open source group chat

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package models

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the GetSavedSnippets200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetSavedSnippets200Response{}

// GetSavedSnippets200Response struct for GetSavedSnippets200Response
type GetSavedSnippets200Response struct {
	Result string `json:"result"`
	Msg    string `json:"msg"`
	// An array of any parameters sent in the request that are not supported by the endpoint.  See [error handling](/api/rest-error-handling#ignored-parameters) documentation for details on this and its change history.
	IgnoredParametersUnsupported []string `json:"ignored_parameters_unsupported,omitempty"`
	// An array of dictionaries containing data on all of the current user's saved snippets.
	SavedSnippets []SavedSnippet `json:"saved_snippets,omitempty"`
}

type _GetSavedSnippets200Response GetSavedSnippets200Response

// NewGetSavedSnippets200Response instantiates a new GetSavedSnippets200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetSavedSnippets200Response(result string, msg string) *GetSavedSnippets200Response {
	this := GetSavedSnippets200Response{}
	this.Result = result
	this.Msg = msg
	return &this
}

// NewGetSavedSnippets200ResponseWithDefaults instantiates a new GetSavedSnippets200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetSavedSnippets200ResponseWithDefaults() *GetSavedSnippets200Response {
	this := GetSavedSnippets200Response{}
	return &this
}

// GetResult returns the Result field value
func (o *GetSavedSnippets200Response) GetResult() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Result
}

// GetResultOk returns a tuple with the Result field value
// and a boolean to check if the value has been set.
func (o *GetSavedSnippets200Response) GetResultOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Result, true
}

// SetResult sets field value
func (o *GetSavedSnippets200Response) SetResult(v string) {
	o.Result = v
}

// GetMsg returns the Msg field value
func (o *GetSavedSnippets200Response) GetMsg() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Msg
}

// GetMsgOk returns a tuple with the Msg field value
// and a boolean to check if the value has been set.
func (o *GetSavedSnippets200Response) GetMsgOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Msg, true
}

// SetMsg sets field value
func (o *GetSavedSnippets200Response) SetMsg(v string) {
	o.Msg = v
}

// GetIgnoredParametersUnsupported returns the IgnoredParametersUnsupported field value if set, zero value otherwise.
func (o *GetSavedSnippets200Response) GetIgnoredParametersUnsupported() []string {
	if o == nil || IsNil(o.IgnoredParametersUnsupported) {
		var ret []string
		return ret
	}
	return o.IgnoredParametersUnsupported
}

// GetIgnoredParametersUnsupportedOk returns a tuple with the IgnoredParametersUnsupported field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSavedSnippets200Response) GetIgnoredParametersUnsupportedOk() ([]string, bool) {
	if o == nil || IsNil(o.IgnoredParametersUnsupported) {
		return nil, false
	}
	return o.IgnoredParametersUnsupported, true
}

// HasIgnoredParametersUnsupported returns a boolean if a field has been set.
func (o *GetSavedSnippets200Response) HasIgnoredParametersUnsupported() bool {
	if o != nil && !IsNil(o.IgnoredParametersUnsupported) {
		return true
	}

	return false
}

// SetIgnoredParametersUnsupported gets a reference to the given []string and assigns it to the IgnoredParametersUnsupported field.
func (o *GetSavedSnippets200Response) SetIgnoredParametersUnsupported(v []string) {
	o.IgnoredParametersUnsupported = v
}

// GetSavedSnippets returns the SavedSnippets field value if set, zero value otherwise.
func (o *GetSavedSnippets200Response) GetSavedSnippets() []SavedSnippet {
	if o == nil || IsNil(o.SavedSnippets) {
		var ret []SavedSnippet
		return ret
	}
	return o.SavedSnippets
}

// GetSavedSnippetsOk returns a tuple with the SavedSnippets field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetSavedSnippets200Response) GetSavedSnippetsOk() ([]SavedSnippet, bool) {
	if o == nil || IsNil(o.SavedSnippets) {
		return nil, false
	}
	return o.SavedSnippets, true
}

// HasSavedSnippets returns a boolean if a field has been set.
func (o *GetSavedSnippets200Response) HasSavedSnippets() bool {
	if o != nil && !IsNil(o.SavedSnippets) {
		return true
	}

	return false
}

// SetSavedSnippets gets a reference to the given []SavedSnippet and assigns it to the SavedSnippets field.
func (o *GetSavedSnippets200Response) SetSavedSnippets(v []SavedSnippet) {
	o.SavedSnippets = v
}

func (o GetSavedSnippets200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetSavedSnippets200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["result"] = o.Result
	toSerialize["msg"] = o.Msg
	if !IsNil(o.IgnoredParametersUnsupported) {
		toSerialize["ignored_parameters_unsupported"] = o.IgnoredParametersUnsupported
	}
	if !IsNil(o.SavedSnippets) {
		toSerialize["saved_snippets"] = o.SavedSnippets
	}
	return toSerialize, nil
}

func (o *GetSavedSnippets200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"result",
		"msg",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetSavedSnippets200Response := _GetSavedSnippets200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varGetSavedSnippets200Response)

	if err != nil {
		return err
	}

	*o = GetSavedSnippets200Response(varGetSavedSnippets200Response)

	return err
}

type NullableGetSavedSnippets200Response struct {
	value *GetSavedSnippets200Response
	isSet bool
}

func (v NullableGetSavedSnippets200Response) Get() *GetSavedSnippets200Response {
	return v.value
}

func (v *NullableGetSavedSnippets200Response) Set(val *GetSavedSnippets200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetSavedSnippets200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetSavedSnippets200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetSavedSnippets200Response(val *GetSavedSnippets200Response) *NullableGetSavedSnippets200Response {
	return &NullableGetSavedSnippets200Response{value: val, isSet: true}
}

func (v NullableGetSavedSnippets200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetSavedSnippets200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
