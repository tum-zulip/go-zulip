...
func (dst *DisplayRecipient) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into UserRecipents
	err = newStrictDecoder(data).Decode(&dst.UserRecipents)
	if err == nil {
		UserRecipents, _ := json.Marshal(dst.UserRecipents)
		if string(UserRecipents) == "{}" { // empty struct
			dst.UserRecipents = nil
		} else {
			if err = validator.Validate(dst.UserRecipents); err != nil {
				dst.UserRecipents = nil
			} else {
				match++
			}
	...
	err = newStrictDecoder(data).Decode(&dst.Name)
	if err == nil {
		jsonString, _ := json.Marshal(dst.Name)
		if string(jsonString) == "{}" { // empty struct
			dst.Name = nil
		} else {
			if err = validator.Validate(dst.Name); err != nil {
				dst.Name = nil
			} else {
				match++
			}
...
func (src DisplayRecipient) MarshalJSON() ([]byte, error) {
	if src.UserRecipents != nil {
		return json.Marshal(&src.UserRecipents)
	}
